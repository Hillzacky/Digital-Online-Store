    ...
    <div class=" container-fluid  container-fixed-lg">
      <div class="inner">
        ...
        <ol class="breadcrumb">  
          ...
          <li class="breadcrumb-item"><a href="{{ route('Instagrams.index') }}"> {{ __('City')}}</a></li>
          <li class="breadcrumb-item active"> {{ __('Create City')}}</li>
        </ol>
        <!-- END BREADCRUMB -->
        <div class="container-md-height">
          <div class="row">
            ...
            <div class="card card-transparent">
              <div class="card-body"> 
                <h3><i class="fas fa-city"></i> {{ __('Add City')}} </h3>
              </div>
            </div>
            <!-- END card -->
          </div>
        ...
        <div class="card card-default">
          ...
          <form action="{{ route('Instagrams.store') }}" method="POST"  role="form" enctype="multipart/form-data"> 
            ...
            <div class="form-group form-group-default required">
              <label for="Title_en">{{ __('City Title')}}</label>
              <input type="text" class="form-control @error('Title_en') is-invalid @enderror" 
                     required="" placeholder="ex: City" id="Title_en" name="Title_en">
              @error('Title_en')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror 
            </div>
            <div class="form-group form-group-default required">
              <label for="body_en">{{ __('City Content')}}</label>
              <input type="text" class="form-control @error('body_en') is-invalid @enderror" 
                     required="" placeholder="ex:Content City" id="body_en" name="body_en">
              @error('body_en')
                <div class="alert alert-danger">{{ $message }}</div>
              @enderror 
            </div>
